...
#define MOSFET8_HW_I2C_SECOND_BASE_ADD	0x20
...
SM_8mosind::SM_8mosind()
{
}

uint8_t SM_8mosind::begin(uint8_t stack)
{
	uint8_t ret = 0;
	if (stack < 0)
	{
	...
	if(	0 != ret)
	{
		_hwAdd = MOSFET8_HW_I2C_SECOND_BASE_ADD + (stack ^ 0x07);
		Wire.begin();
		Wire.beginTransmission(_hwAdd);
		Wire.write(MOSFET8_OUTPORT_REG_ADD);
		Wire.write(0xff); //make all High
...
uint8_t SM_8mosind::begin()
{
	return begin(0);
}
uint8_t SM_8mosind::rawRead(uint8_t *val)
{
	Wire.beginTransmission(_hwAdd);
	Wire.write(MOSFET8_OUTPORT_REG_ADD);
...
uint8_t SM_8mosind::rawWrite(uint8_t val)
{
...
uint8_t SM_8mosind::writeChannel(uint8_t channel, uint8_t state)
{
	uint8_t val = 0;
...
uint8_t SM_8mosind::writeChannel(int channel, int state)
{
	return writeChannel((uint8_t)channel, (uint8_t)state);
}

uint8_t SM_8mosind::readChannel(uint8_t channel)
{
	uint8_t val = 0;
...
uint8_t SM_8mosind::writeAll(uint8_t val)
{
	uint8_t outVal = 0;
	uint8_t i = 0;
